...
#include "testlibs/minitest/assertion_strings.hpp"  // IWYU pragma: export
...
#include "testlibs/minitest/line_number.hpp"      // IWYU pragma: export
#include "testlibs/minitest/matcher.hpp"          // IWYU pragma: export
#include "testlibs/minitest/minitest_output.hpp"  // IWYU pragma: export
#include "testlibs/minitest/test_context.hpp"     // IWYU pragma: export
...
constexpr auto
do_constexpr_assert_eq(Lhs&& lhs, Rhs&& rhs, char const* expr, char const* file, line_number line) noexcept -> bool {
...
#endif
  auto const result = static_cast<Lhs&&>(lhs) == static_cast<Rhs&&>(rhs);
#if defined(__clang__)
  _Pragma("clang diagnostic pop") static_assert(true, "");
#elif defined(__GNUC__)
  _Pragma("GCC diagnostic pop") static_assert(true, "");
#endif
  return do_assert_true(result, expr, file, line);
}
...
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define CONSTEXPR_ASSERT_EQ(...) \
  ::testing::do_constexpr_assert_eq(__VA_ARGS__, "eq(" #__VA_ARGS__ ")", __FILE__, __LINE__)

/// @brief Macro to assert that an expression is @c true
/// NOTE: Usable within a constant expression
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define CONSTEXPR_ASSERT(...) CONSTEXPR_ASSERT_EQ(true, (__VA_ARGS__))

/// @brief Macro to assert that an expression is @c false
/// NOTE: Usable within a constant expression
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define CONSTEXPR_ASSERT_FALSE(...) CONSTEXPR_ASSERT_EQ(false, (__VA_ARGS__))
...
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define ASSERT_THAT(value_expression, matcher)                \
  if (true) { /* NOLINT(readability-simplify-boolean-expr) */ \
    ASSERT_TRUE((matcher)((value_expression)));               \
  } else                                                      \
    (+::testing::stream_insertion_sink{})
...
// NOLINTNEXTLINE(cppcoreguidelines-macro-usage)
#define EXPECT_THAT(value_expression, matcher)                \
  if (true) { /* NOLINT(readability-simplify-boolean-expr) */ \
    EXPECT_TRUE((matcher)((value_expression)));               \
  } else                                                      \
    (+::testing::stream_insertion_sink{})
