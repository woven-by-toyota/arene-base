...
TEST_F(BindFrontTest, ReturnsAnInvocableWhichIsTheInputInvocableWithTheFirstNArgumentsCurried) {
  ...
  STATIC_ASSERT_TRUE(arene::base::is_invocable_v<  //
                     decltype(bind_front(&type_with_member_function::do_stuff)),
...
TEST_F(BindFrontTest, BoundArgumentMismatchIsNotAnErrorButDoesNotHaveCallOverloads) {
  ...
  auto const extra_arg_mf =
      bind_front(&type_with_member_function::do_stuff, type_with_member_function{}, arg1t{}, arg2t{}, arg3t{}, 0U);
  ...
  auto const wrong_arg_mf =
      bind_front(&type_with_member_function::do_stuff, type_with_member_function{}, arg1t{}, arg3t{}, arg2t{});
...
TEST_F(BindFrontTest, TheNoexceptnessOfTheInputCallableIsPreserved) {
  ...
  using some_args_curried_str =
      decltype(bind_front(std::declval<callable_with_arg_and_ref_based_noexcept>(), user_defined_type{}));
  STATIC_ASSERT_EQ(
      (is_nothrow_invocable_v<some_args_curried_str&, user_defined_type>),
      (is_nothrow_invocable_v<callable_with_arg_and_ref_based_noexcept&, user_defined_type, user_defined_type>)
  );
  STATIC_ASSERT_EQ(
      (is_nothrow_invocable_v<some_args_curried_str const&, user_defined_type>),
      (is_nothrow_invocable_v<callable_with_arg_and_ref_based_noexcept const&, user_defined_type, user_defined_type>)
  );
  STATIC_ASSERT_EQ(
      (is_nothrow_invocable_v<some_args_curried_str&&, user_defined_type>),
      (is_nothrow_invocable_v<callable_with_arg_and_ref_based_noexcept&&, user_defined_type, user_defined_type>)
  ...
  using all_args_curried_str = decltype(bind_front(
      std::declval<callable_with_arg_and_ref_based_noexcept>(),
      user_defined_type{},
      user_defined_type{}
  ));
  STATIC_ASSERT_EQ(
      (is_nothrow_invocable_v<all_args_curried_str&>),
      (is_nothrow_invocable_v<callable_with_arg_and_ref_based_noexcept&, user_defined_type, user_defined_type>)
  );
  STATIC_ASSERT_EQ(
      (is_nothrow_invocable_v<all_args_curried_str const&>),
      (is_nothrow_invocable_v<callable_with_arg_and_ref_based_noexcept const&, user_defined_type, user_defined_type>)
  );
  STATIC_ASSERT_EQ(
      (is_nothrow_invocable_v<all_args_curried_str&&>),
...
TEST_F(BindFrontTest, TheReferenceQualificationOfTheReturnedCallableIsAppliedToTheBoundArguments) {
  ...
  auto const bound_first_arg_initial_state = int_wrapper{1};

  auto const first_arg_must_be_rvalue = [](int_wrapper&& lhs, int_wrapper const& rhs) -> int_wrapper {
  ...
  // NOLINTNEXTLINE(hicpp-move-const-arg)
  EXPECT_EQ(std::move(only_invocable_if_moved)(int_wrapper{2}), bound_first_arg_initial_state + int_wrapper{2});

  auto const first_arg_must_be_non_const_lvalue = [](int_wrapper& lhs, int_wrapper const& rhs) -> int_wrapper {
    lhs += rhs;
    return lhs;
  };
...
/// @test the result of back_bind can be used with bind_front
TEST_F(BindFrontTest, PartiallyAppliedClosureIsBindable) {
  auto const f = bind_front(std::minus<>{}, 2);
  auto const dummy = [](auto const& func, auto const& arg) { return func(arg); };
  auto const g = bind_front(dummy, f);

  EXPECT_EQ(-1, g(3));
}

/// @test the result of back_bind can be used with bind_front where the same argument type is bound
TEST_F(BindFrontTest, PartiallyAppliedClosureIsBindableWithSameBoundArgType) {
  auto const f = bind_front(std::minus<>{}, 2);
  auto const g = bind_front(f, 3);
...
/// @test bind_front is a function object
TEST_F(BindFrontTest, IsFunctionObject) { std::ignore = bind_front; }
