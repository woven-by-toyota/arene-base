...
#ifndef INCLUDE_GUARD_ARENE_BASE_ARENE_BASE_FUNCTIONAL_BIND_FRONT_HPP_
#define INCLUDE_GUARD_ARENE_BASE_ARENE_BASE_FUNCTIONAL_BIND_FRONT_HPP_
...
class bind_front_policy;

/// @brief Implementation helper for @c arene::base::bind_front intended to be used as a policy for
...
class bind_front_policy<NBound, std::index_sequence<Is...>> {
 public:
  /// @brief Invokes the provided function as expression-equivalent to @c func(bound_args...,call_args...) .
  ///
...
class bind_front_t
    : functional_detail::
          perfect_forward_call_wrapper<bind_front_policy<sizeof...(BoundArgs)>, BoundFunc, BoundArgs...> {
  /// @brief specialization of perfect_forward_call_wrapper that binds arguments to the front
  using impl_type =
      functional_detail::perfect_forward_call_wrapper<bind_front_policy<sizeof...(BoundArgs)>, BoundFunc, BoundArgs...>;
...
/// @brief function object implementing bind_front
class bind_front_fn {
 public:
  /// @brief Produces a call wrapper which allows invoking an input callable with its first N arguments bound to @c args
  /// .
  ///
  ...
  /// .
  ///         <c> bind_front(bound_func, bound_args...)(call_args...) </c> is expression-equivalent to
  ///         <c> invoke(bound_func, bound_args..., call_args...) </c> . The @c noexcept -ness of @c bound_func is
  ...
  /// bind_front(f,args...) , then invoking @c std::move(g)(call_args...) will allow the bound arguments to be moved
  /// into the bound callable. Similarly, if @c f where to have non-const lvalue parameters for at the position of bound
  /// arguments, then invoking @c g(call_args..) when @c g is a non-const lvalue will result in the instances of the
  /// bound arguments held in the wrapper being passed to @c f , similar to a @c mutable lambda with a capture block.
  ///
  /// If a caller wishes to maintain "reference semantics" of the bound arguments when it can be assured the lifetime of
  /// the bound arguments will exceed the lifetime of the returned callable, this can be achieved by wrapping the
  /// argument in @c std::reference_wrapper .
  ...
  template <typename BoundFunc, typename... BoundArgs>
  constexpr auto operator()(BoundFunc&& bound_func, BoundArgs&&... bound_args) const noexcept(
  ...
  ) -> bind_front_detail::bind_front_t<std::decay_t<BoundFunc>, std::decay_t<BoundArgs>...> {
    static_assert(
    ...
    return bind_front_detail::bind_front_t<std::decay_t<BoundFunc>, std::decay_t<BoundArgs>...>{
...
/// @def arene::base::bind_front
/// @copydoc arene::base::bind_front_detail::bind_front_fn::operator()
...
// parasoft-end-suppress AUTOSAR-A2_8_1-a-2 "False positive: also defines arene::base::bind_front"

#endif  // INCLUDE_GUARD_ARENE_BASE_ARENE_BASE_FUNCTIONAL_BIND_FRONT_HPP_
