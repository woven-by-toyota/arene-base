...
static_assert(arene::base::tuple_like_detail::get_result_is_tuple_element_ref<arene::base::array<int, 1U>, 0U>, "");
...
/// @brief Move-only type used to verify is_tuple_like accepts tuple-likes holding non-copyable elements
...
using test_types = ::testing::Types<
    ...
    // Tuple-likes holding move-only elements must be tuple-like: get<I>(t&) returns
    // `move_only&`, which binds to `move_only const&` but is not copy-constructible into
    // a `move_only` value. The convertibility-to-`tuple_element_t const&` check accepts them.
    expectation<std::tuple<move_only>, true, false>,
    expectation<std::tuple<move_only, move_only>, true, true>,
    expectation<std::pair<move_only, int>, true, true>,
    expectation<arene::base::array<move_only, 3>, true, false>>;
