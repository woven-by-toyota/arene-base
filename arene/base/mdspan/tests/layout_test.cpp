...
using arene::base::testing::make_full_extents;
using arene::base::testing::make_full_range_extent_slice;
using arene::base::testing::make_mapping_with_extents;
...
/// @test @c submdspan_mapping with all @c full_extent slices returns the same
/// mapping type with the same extents and a zero offset
CONSTEXPR_TYPED_TEST(LayoutTest, SubmdspanMappingFullExtents) {
  using extents_type = typename TypeParam::extents_type;

  auto const exts = test::make_expanded_extents<extents_type>();
  auto const mapping = make_mapping_with_extents<TypeParam>(exts);

  auto const result = arene::base::apply(  //
      arene::base::bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping),
      make_full_extents<extents_type::rank()>()
  );

  static_assert(  //
      std::is_same<TypeParam, decltype(result.mapping)>::value,
      "must preserve layout type"
  );
...
/// @test @c submdspan_mapping with all 0 slices returns a rank-0 mapping with a zero offset
CONDITIONAL_CONSTEXPR_TYPED_TEST(
    LayoutTest,
    SubmdspanMappingCollapsingLowerBound,
  ...
  auto const result = arene::base::apply(  //
      arene::base::bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping),
      arene::base::array<typename extents_type::index_type, extents_type::rank()>{}
  );

  CONSTEXPR_ASSERT_EQ(result.mapping, decltype(result.mapping){});
  CONSTEXPR_ASSERT_EQ(result.offset, static_cast<std::size_t>(typename extents_type::index_type{}));
}

/// @test @c submdspan_mapping where each slice is the max extent value per rank
/// returns an empty mapping and an offset equal to those indices applied to the mapping
CONDITIONAL_CONSTEXPR_TYPED_TEST(
    LayoutTest,
    SubmdspanMappingCollapsingMaxIndices,
    arene::base::testing::all_positive_static_extents(typename TypeParam::extents_type{})
) {
  using extents_type = typename TypeParam::extents_type;
  using index_type = typename TypeParam::index_type;
  ...
  auto const max_indices = arene::base::tuple_detail::tuple_transform_to_array<index_type>(
      arene::base::mdspan_detail::to_array(exts),
      arene::base::bind_back(std::minus<index_type>{}, index_type{1})
  );
  ...
  CONSTEXPR_ASSERT_EQ(result.mapping, decltype(result.mapping){});
  CONSTEXPR_ASSERT_EQ(result.offset, static_cast<std::size_t>(arene::base::apply(mapping, max_indices)));
}

/// @test @c submdspan_mapping where each slice is an @c extent_slice that spans
/// the full range is equal to the result of @c submdspan_mapping with all @c full_extent slices
CONSTEXPR_TYPED_TEST(LayoutTest, SubmdspanMappingFullRangeSlices) {
  using extents_type = typename TypeParam::extents_type;
  ...
  auto const result1 = arene::base::apply(  //
      arene::base::bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping),
      make_full_extents<extents_type::rank()>()
  );

  using slice_maker = make_full_range_extent_slice<index_type>;
  using extent_slice_type = typename slice_maker::extent_slice_type;

  auto const full_extent_slices = arene::base::tuple_detail::tuple_transform_to_array<extent_slice_type>(
      arene::base::mdspan_detail::to_array(exts),
      slice_maker{}
  );

  auto const result2 = arene::base::apply(  //
      arene::base::bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping),
      full_extent_slices
  );

  CONSTEXPR_ASSERT_EQ(result1.mapping, result2.mapping);
  CONSTEXPR_ASSERT_EQ(result1.offset, result2.offset);
}

/// @test @c submdspan_mapping is invocable with an extent_slice for rank-1
CONDITIONAL_TYPED_TEST(
    LayoutTest,
    SubmdspanMappingInvocableWithExtentSliceRank1,
    (TypeParam::extents_type::rank() == 1) && test::all_extents_greater_than_1(TypeParam{}.extents())
) {
  using index_type = typename TypeParam::index_type;
  using slice_type = arene::base::extent_slice<index_type>;

  static_assert(
      arene::base::
          is_invocable_v<decltype(arene::base::mdspan_detail::do_submdspan_mapping), TypeParam const&, slice_type>,
      "must be invocable with extent_slice"
  );
}

/// @test @c submdspan_mapping is invocable with mixed extent_slice and index slices for rank-2
CONDITIONAL_TYPED_TEST(
    LayoutTest,
    SubmdspanMappingInvocableWithMixedSlicesRank2,
    (TypeParam::extents_type::rank() == 2) && test::all_extents_greater_than_1(TypeParam{}.extents())
) {
  using index_type = typename TypeParam::index_type;
  using slice_type = arene::base::extent_slice<index_type>;
  ...
  static_assert(
      arene::base::is_invocable_v<
          decltype(arene::base::mdspan_detail::do_submdspan_mapping),
          TypeParam const&,
          slice_type,
          index_type>,
      "must be invocable with extent_slice, index"
  );
  ...
  static_assert(
      arene::base::is_invocable_v<
          ...
          index_type,
          slice_type>,
      "must be invocable with index, extent_slice"
  );
  ...
  static_assert(
      arene::base::is_invocable_v<
          ...
          slice_type,
          slice_type>,
      "must be invocable with extent_slice, extent_slice"
  );
}

/// @test @c submdspan_mapping is invocable with mixed extent_slice and index slices for rank-3
CONDITIONAL_TYPED_TEST(
    LayoutTest,
    SubmdspanMappingInvocableWithMixedSlicesRank3,
    (TypeParam::extents_type::rank() == 3) && test::all_extents_greater_than_1(TypeParam{}.extents())
) {
  using index_type = typename TypeParam::index_type;
  using slice_type = arene::base::extent_slice<index_type>;
  ...
  static_assert(
      arene::base::is_invocable_v<
          ...
          index_type,
          index_type>,
      "must be invocable with extent_slice, index, index"
  );
  ...
  static_assert(
      arene::base::is_invocable_v<
          ...
          slice_type,
          index_type>,
      "must be invocable with index, extent_slice, index"
  );
  ...
  static_assert(
      arene::base::is_invocable_v<
          ...
          index_type,
          slice_type>,
      "must be invocable with index, index, extent_slice"
  );
  ...
  static_assert(
      arene::base::is_invocable_v<
          ...
          slice_type,
          slice_type>,
      "must be invocable with all extent_slices"
  );
  ...
  static_assert(
      arene::base::is_invocable_v<
          ...
          index_type,
          slice_type>,
      "must be invocable with extent_slice, index, extent_slice"
  );
}

/// @test @c submdspan_mapping is not invocable if the slice count is not equal to the rank
TYPED_TEST(LayoutTest, NotInvocableIfSliceCountNotEqualToRank) {
  using extents_type = typename TypeParam::extents_type;

  static_assert(
      arene::base::type_lists::concat_t<
          arene::base::is_invocable<decltype(arene::base::mdspan_detail::do_submdspan_mapping), TypeParam const&>,
...
/// @test @c submdspan_mapping is not invocable with non-canonical slice types
TYPED_TEST(LayoutTest, NotInvocableWithNonCanonicalSliceType) {
  using extents_type = typename TypeParam::extents_type;
  using index_type = typename extents_type::index_type;

  // nullptr_t is not a canonical slice type
  static_assert(
      !arene::base::
          is_invocable_v<decltype(arene::base::mdspan_detail::do_submdspan_mapping), TypeParam const&, std::nullptr_t>,
      "must not be invocable with nullptr_t"
  );

  // range_slice is a valid submdspan slice type but not a canonical slice type
  static_assert(
      !arene::base::is_invocable_v<
          decltype(arene::base::mdspan_detail::do_submdspan_mapping),
          TypeParam const&,
          arene::base::range_slice<index_type>>,
      "must not be invocable with range_slice"
  );
}

/// @test @c submdspan_mapping is unconditionally noexcept with canonical slice types
CONDITIONAL_TYPED_TEST(LayoutTest, SubmdspanMappingIsUnconditionallyNoexcept, TypeParam::extents_type::rank() >= 1U) {
  using extents_type = typename TypeParam::extents_type;
  using index_type = typename extents_type::index_type;
  ...
  static_assert(
      arene::base::type_lists::concat_t<
          arene::base::
              is_nothrow_invocable<decltype(arene::base::mdspan_detail::do_submdspan_mapping), TypeParam const&>,
          arene::base::repeat_type_t<extents_type::rank(), arene::base::type_list, index_type>>::value,
      "must be noexcept with index slices"
  );
  ...
  static_assert(
      arene::base::type_lists::concat_t<
          arene::base::
              is_nothrow_invocable<decltype(arene::base::mdspan_detail::do_submdspan_mapping), TypeParam const&>,
          arene::base::repeat_type_t<extents_type::rank(), arene::base::type_list, arene::base::full_extent_t>>::value,
      "must be noexcept with full_extent slices"
  );
}
}  // namespace
