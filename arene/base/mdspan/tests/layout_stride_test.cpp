...
CONSTEXPR_TEST(LayoutStrideTest, RequiredSpanSizeIsZeroIfExtentsSpaceIsZero) {
  ...
  CONSTEXPR_ASSERT_EQ(0U, static_cast<std::size_t>(mapping.required_span_size()));
}
...
/// @test @c submdspan_mapping where each slice is an @c extent_slice with stride
/// 2 that spans the full range has:
/// * a mapping:
/// ** where the strides are doubled (for slices with more than one extent)
/// ** and the extents are halved
/// * and the offset is zero
CONSTEXPR_TYPED_TEST(LayoutStrideTest, SubmdspanMappingFullRangeSlicesStride2) {
  using extents_type = typename TypeParam::extents_type;
  using index_type = typename extents_type::index_type;

  auto const exts = test::make_expanded_extents<extents_type>();
  auto const right_mapping = arene::base::layout_right::mapping<extents_type>{exts};
  auto const mapping = TypeParam{right_mapping};

  using extent_slice_type = extent_slice<index_type, index_type, std::integral_constant<index_type, 2>>;

  struct to_strided_extent_slice {
    constexpr auto operator()(index_type ext) const -> extent_slice_type {
      // extent_in_strided_interval: if ext == 0, return 0; else return 1 + (ext - 1) / 2
      auto const extent =
          (ext == index_type{})
              ? index_type{}
              : static_cast<index_type>(index_type{1} + static_cast<index_type>((ext - index_type{1}) / index_type{2}));
      return {index_type{}, extent};
    }
  };

  auto const strided_extent_slices = arene::base::tuple_detail::tuple_transform_to_array<extent_slice_type>(
      arene::base::mdspan_detail::to_array(exts),
      to_strided_extent_slice{}
  );

  auto const result = apply(  //
      bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping),
      strided_extent_slices
  );

  struct double_stride_if_slice_extent_greater_than_one {
    constexpr auto operator()(index_type stride, extent_slice_type slice) const -> index_type {
      if (slice.extent > index_type{1}) {
        return arene::base::mdspan_detail::representable_cast<index_type>(index_type{2} * stride);
      }
      return stride;
    }
  };
  ...
  CONSTEXPR_ASSERT_EQ(
      arene::base::tuple_detail::tuple_transform_to_array<index_type>(
          mapping.strides(),
          strided_extent_slices,
          double_stride_if_slice_extent_greater_than_one{}
      ),
      result.mapping.strides()
  );
  CONSTEXPR_ASSERT_EQ(  //
      apply(bind_front(arene::base::submdspan_extents, exts), strided_extent_slices),
      result.mapping.extents()
  );
  CONSTEXPR_ASSERT_EQ(0U, result.offset);
}

/// @test @c submdspan_mapping where each slice is an @c extent_slice that spans
/// half the full range has:
/// * a mapping where the extents are halved
/// * and the offset is zero
CONSTEXPR_TYPED_TEST(LayoutStrideTest, SubmdspanMappingHalfRangeSlices) {
  using extents_type = typename TypeParam::extents_type;
  using index_type = typename extents_type::index_type;
  ...
  using extent_slice_type = extent_slice<index_type, index_type, std::integral_constant<index_type, 1>>;

  struct to_half_extent_slice {
    constexpr auto operator()(index_type ext) const -> extent_slice_type {
      return {index_type{}, static_cast<index_type>(ext / index_type{2})};
    }
  };

  auto const half_extent_slices = arene::base::tuple_detail::tuple_transform_to_array<extent_slice_type>(
      arene::base::mdspan_detail::to_array(exts),
      to_half_extent_slice{}
  );
  ...
  CONSTEXPR_ASSERT_EQ(mapping.strides(), result.mapping.strides());
  CONSTEXPR_ASSERT_EQ(
      apply(bind_front(arene::base::submdspan_extents, exts), half_extent_slices),
      result.mapping.extents()
  );
  CONSTEXPR_ASSERT_EQ(0U, result.offset);
}

/// @test @c submdspan_mapping where each slice is a full @c extent_slice with
/// runtime values has a result mapping with all dynamic extents
CONSTEXPR_TYPED_TEST(LayoutStrideTest, SubmdspanMappingRuntimeExtentSlicesReturnsMappingTypeWithDextents) {
  using extents_type = typename TypeParam::extents_type;
  ...
  struct to_runtime_full_extent_slice {
    constexpr auto operator()(index_type ext) const -> extent_slice_type { return {index_type{}, ext, index_type{1}}; }
  };

  auto const runtime_full_extent_slices = arene::base::tuple_detail::tuple_transform_to_array<extent_slice_type>(
      arene::base::mdspan_detail::to_array(exts),
      to_runtime_full_extent_slice{}
  );

  auto const result = apply(  //
      bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping),
      runtime_full_extent_slices
  );

  static_assert(     //
      std::is_same<  //
          layout_stride::mapping<dextents<index_type, extents_type::rank()>>,
          decltype(result.mapping)>::value,
      "must be the same type"
  );

  CONSTEXPR_ASSERT_EQ(mapping, result.mapping);
  CONSTEXPR_ASSERT_EQ(0U, result.offset);
}

/// @brief A layout whose mapping publicly inherits from @c layout_stride::mapping, used to verify that
/// @c submdspan_mapping is found via ADL for public descendants of @c layout_stride::mapping
struct derived_stride_layout {
  ...
  class mapping : public layout_stride::mapping<Extents> {
   ...
   public:
    ...
    using layout_type = derived_stride_layout;

    constexpr mapping() noexcept = default;

    constexpr explicit mapping(extents_type const& exts) noexcept
        : base_type(exts) {}
  };
};

/// @test @c submdspan_mapping is found via ADL for a type that publicly inherits from @c layout_stride::mapping
TEST(DerivedFromLayoutStrideTest, SubmdspanMappingFoundViaAdl) {
  using extents_type = extents<std::size_t, 3, 4>;
  using derived_mapping = derived_stride_layout::mapping<extents_type>;

  derived_mapping const src{};

  auto const result = arene::base::mdspan_detail::do_submdspan_mapping(
      src,
      extent_slice<std::size_t, std::size_t>{0U, 2U},
      full_extent_t{}
  );

  // The result should be a layout_stride mapping (the layout_stride hidden friend produces layout_stride results)
  using expected_sub_extents = arene::base::extents<std::size_t, arene::base::dynamic_extent, 4>;
  static_assert(
      std::is_same<layout_stride::mapping<expected_sub_extents>, decltype(result.mapping)>::value,
      "submdspan_mapping of a derived-from-layout_stride type must produce a layout_stride::mapping"
  );

  EXPECT_EQ(result.mapping.extents().extent(0), 2U);
  EXPECT_EQ(result.mapping.extents().extent(1), 4U);
