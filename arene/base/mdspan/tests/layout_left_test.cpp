...
CONSTEXPR_TYPED_TEST(LayoutLeftTest, ConstructFromLayoutRightMapping) {
  ...
  using right_type_if_convertible = std::conditional_t<rank <= 1U, right_mapping_type, TypeParam>;
...
TYPED_TEST(LayoutLeftTest, ConstructFromLayoutRightMappingWithOtherExtents) {
  ...
  // Workaround for lack of constexpr if to make sure the block below always compiles
  using imp_type_if_convertible = std::conditional_t<rank <= 1U, right_mapping_implicit_t, TypeParam>;
  using exp_type_if_convertible = std::conditional_t<rank <= 1U, right_mapping_explicit_t, TypeParam>;
