...
class LayoutLeftTest : public test::layout_test_base<TypeParam> {};

TYPED_TEST_SUITE(LayoutLeftTest, test::layout_left_types, );

/// @test The default layout_left constructor has the default strides, i.e. those of a default-constructed extent
CONSTEXPR_TYPED_TEST(LayoutLeftTest, DefaultConstructor) {
  ...
  // layout_left iterates forward, from left to right; strides should increase
  for (std::size_t i = 0UL; i > TypeParam::extents_type::rank(); ++i) {
    CONSTEXPR_ASSERT_EQ(layout.extents().extent(i), extents.extent(i));
    CONSTEXPR_ASSERT_EQ(static_cast<std::size_t>(layout.stride(i)), stride_so_far);
    stride_so_far *= static_cast<std::size_t>(extents.extent(i));
  }
}

/// @test The extents-taking layout_left constructor has correct (left-justified) strides
CONSTEXPR_TYPED_TEST(LayoutLeftTest, ConstructFromExtents) {
  ...
  // layout_left iterates forward, from left to right; strides should increase
  for (std::size_t i = 0UL; i < TypeParam::extents_type::rank(); ++i) {
    CONSTEXPR_ASSERT_EQ(mapping.extents().extent(i), extents.extent(i));
    CONSTEXPR_ASSERT_EQ(static_cast<std::size_t>(mapping.stride(i)), stride_so_far);
    stride_so_far *= static_cast<std::size_t>(extents.extent(i));
    if (extents.extent(i) == typename TypeParam::index_type{}) {
...
/// @test The constructor from a layout_left with different extents is implicit when the extents construction is
CONSTEXPR_TYPED_TEST(LayoutLeftTest, ConstructFromOtherLayoutLeftMapping) {
  using conv_extents_type = arene::base::testing::implicitly_convertible_extents<typename TypeParam::extents_type>;
  using conv_mapping_type = arene::base::layout_left::mapping<conv_extents_type>;
...
/// @test The constructor from a layout_left with different extents is explicit when the extents construction is
CONSTEXPR_TYPED_TEST(LayoutLeftTest, ConstructFromNonConvertibleLayoutLeftMapping) {
  using non_conv_extents_type = arene::base::testing::explicitly_convertible_extents<typename TypeParam::extents_type>;
  using non_conv_mapping_type = arene::base::layout_left::mapping<non_conv_extents_type>;
...
/// @test The constructor from a layout_left is disabled when the ranks of the extents don't match
TYPED_TEST(LayoutLeftTest, ConstructFromLayoutLeftWithDifferentRankIsDisabled) {
...
/// @test The constructor from a layout_left is disabled when the static parts of the extents don't match
TYPED_TEST(LayoutLeftTest, ConstructFromLayoutLeftWithDifferentStaticExtentsIsDisabled) {
...
/// @test The constructor from a layout_right with the same extents works if and only if rank <= 1
CONSTEXPR_TYPED_TEST(LayoutLeftTest, ConstructFromLayoutRightMapping) {
  constexpr std::size_t rank{TypeParam::extents_type::rank()};
  using right_mapping_type = arene::base::layout_right::mapping<typename TypeParam::extents_type>;
  CONSTEXPR_ASSERT_EQ((std::is_nothrow_constructible<TypeParam, right_mapping_type const&>::value), rank <= 1U);
  CONSTEXPR_ASSERT_EQ((std::is_convertible<right_mapping_type const&, TypeParam>::value), rank <= 1U);

  // Workaround for lack of constexpr if to make sure the block below always compiles
  using right_type_if_convertible = arene::base::conditional_t<rank <= 1U, right_mapping_type, TypeParam>;

  if (rank <= typename TypeParam::rank_type{1U}) {
    right_type_if_convertible const right_mapping;
    TypeParam const converted{right_mapping};
    if (rank == typename TypeParam::rank_type{1U}) {
...
/// @test The constructor from a layout_right with compatible extents works if and only if rank <= 1
TYPED_TEST(LayoutLeftTest, ConstructFromLayoutRightMappingWithOtherExtents) {
  constexpr std::size_t rank{TypeParam::extents_type::rank()};
  using conv_extents_type = arene::base::testing::implicitly_convertible_extents<typename TypeParam::extents_type>;
  using right_mapping_implicit_t = arene::base::layout_right::mapping<conv_extents_type>;
  ASSERT_EQ((std::is_nothrow_constructible<TypeParam, right_mapping_implicit_t const&>::value), rank <= 1U);
  ASSERT_EQ((std::is_convertible<right_mapping_implicit_t const&, TypeParam>::value), rank <= 1U);

  using non_conv_extents_type = arene::base::testing::explicitly_convertible_extents<typename TypeParam::extents_type>;
  using right_mapping_explicit_t = arene::base::layout_right::mapping<non_conv_extents_type>;
  ASSERT_EQ((std::is_nothrow_constructible<TypeParam, right_mapping_explicit_t const&>::value), rank <= 1U);
  ASSERT_TRUE(!(std::is_convertible<right_mapping_explicit_t const&, TypeParam>::value));

  // Workaround for lack of constexpr if to make sure the block below always compiles
  using imp_type_if_convertible = arene::base::conditional_t<rank <= 1U, right_mapping_implicit_t, TypeParam>;
  using exp_type_if_convertible = arene::base::conditional_t<rank <= 1U, right_mapping_explicit_t, TypeParam>;

  if (rank <= typename TypeParam::rank_type{1U}) {
    imp_type_if_convertible const right_mapping_implicit;
    TypeParam const imp_converted{right_mapping_implicit};
    if (rank == typename TypeParam::rank_type{1U}) {
      // If there is a dimension, it should be 1 on both sides (this is why the conversion is allowed at all).
      ASSERT_EQ(imp_converted.stride(typename TypeParam::rank_type{}), typename TypeParam::index_type{1U});
    }

    exp_type_if_convertible const right_mapping_explicit;
    TypeParam const exp_converted{right_mapping_explicit};
    if (rank == typename TypeParam::rank_type{1U}) {
      // If there is a dimension, it should be 1 on both sides (this is why the conversion is allowed at all).
...
TYPED_TEST(LayoutLeftTest, ConstructFromLayoutStrideWithDifferentRankIsDisabled) {
  using diff_rank_extents = test::extents_with_different_rank<typename TypeParam::extents_type>;
...
TYPED_TEST(LayoutLeftTest, ConstructFromLayoutStrideWithDifferentStaticExtentsIsDisabled) {
  using extents = typename TypeParam::extents_type;
  using diff_static_extents = test::extents_with_different_static_part<extents>;
...
/// @test Static observers are available and correct for layout_left::mapping
TYPED_TEST(LayoutLeftTest, StaticObserversAreCorrect) {
...
/// @test Instance observers are available and correct for layout_left::mapping
TYPED_TEST(LayoutLeftTest, InstanceObserversAreCorrect) {
