...
struct implicitly_convertible_extents_impl<arene::base::extents<IndexType, Extents...>> {
  ...
  using smaller = std::conditional_t<
      std::is_same<unsigned_idx, std::uint64_t>::value,
      std::uint32_t,
      std::conditional_t<std::is_same<unsigned_idx, std::uint32_t>::value, std::uint16_t, std::uint8_t>>;
  using convertible_idx = std::conditional_t<std::is_signed<IndexType>::value, std::make_signed_t<smaller>, smaller>;
...
struct explicitly_convertible_extents_impl<arene::base::extents<IndexType, Extents...>> {
  ...
  using unsigned_idx = std::make_unsigned_t<IndexType>;
  using larger = std::conditional_t<
      std::is_same<unsigned_idx, std::uint8_t>::value,
      std::uint16_t,
      std::conditional_t<std::is_same<unsigned_idx, std::uint16_t>::value, std::uint32_t, std::uint64_t>>;
  using new_idx = std::conditional_t<std::is_signed<IndexType>::value, std::make_signed_t<larger>, larger>;
