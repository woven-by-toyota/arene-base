...
/// @test Invoking submdspan_mapping where the slice upper bound exceeds the
/// valid slice range causes a precondition violation
TYPED_TEST(LayoutDeathTest, SubmdspanMappingInvalidSliceUpperBound) {
  ...
  auto const extents = test::make_expanded_extents<extents_type>();
  auto const mapping = arene::base::testing::make_mapping_with_extents<TypeParam>(extents);

  for (auto dim : sequential_values<std::size_t, rank>) {
    auto const integer_slices = [dim, &extents] {
      using arene::base::array;

      auto slices = array<index_type, rank>{};
      slices[dim] = extents.extent(dim);
      return slices;
    }();

    ASSERT_DEATH(
        apply(bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping), integer_slices),
        "Precondition"
    );

    auto const extent_slices = [dim, &extents] {
      using arene::base::extent_slice;
      using arene::base::mdspan_detail::to_array;
      ...
      auto slices = tuple_transform_to_array<extent_slice_type>(to_array(extents), [](index_type ext) {
        return extent_slice_type{{}, ext};
      });
      ++slices[dim].extent;
      return slices;
    }();
    ...
    ASSERT_DEATH(
        apply(bind_front(arene::base::mdspan_detail::do_submdspan_mapping, mapping), extent_slices),
        "Precondition"
    );
  }
}
