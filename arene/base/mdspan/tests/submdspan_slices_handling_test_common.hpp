...
TYPED_TEST_P(SubmdspanSliceHandlingCommon, Noexcept) {
  ...
  struct convertible_to_full_extent {
    // NOLINTNEXTLINE(hicpp-explicit-conversions)
    operator full_extent_t() const noexcept(false) { return full_extent; }
  };

  ASSERT_FALSE(is_noexcept_with(convertible_to_full_extent{}, int{}));
  ASSERT_FALSE(is_noexcept_with(int{}, convertible_to_full_extent{}));
  ...
  ASSERT_FALSE(is_noexcept_with(convertible_to_int{}, int{}));
  ASSERT_FALSE(is_noexcept_with(int{}, convertible_to_int{}));

  ASSERT_TRUE(is_noexcept_with(std::pair<int, int>{}, int{}));

  ASSERT_FALSE(is_noexcept_with(std::pair<int, convertible_to_int>{}, int{}));
  ASSERT_FALSE(is_noexcept_with(std::pair<convertible_to_int, int>{}, int{}));
}
