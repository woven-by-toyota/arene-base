...
CONSTEXPR_TYPED_TEST(LayoutRightTest, ConstructFromLayoutLeftMapping) {
  ...
  using left_type_if_convertible = arene::base::conditional_t<rank <= 1U, left_mapping_type, TypeParam>;
...
TYPED_TEST(LayoutRightTest, ConstructFromLayoutLeftMappingWithOtherExtents) {
  ...
  // Workaround for lack of constexpr if to make sure the block below always compiles
  using imp_type_if_convertible = arene::base::conditional_t<rank <= 1U, left_mapping_implicit_t, TypeParam>;
  using exp_type_if_convertible = arene::base::conditional_t<rank <= 1U, left_mapping_explicit_t, TypeParam>;
