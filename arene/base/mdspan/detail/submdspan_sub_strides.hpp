...
template <class IndexType, class LayoutMapping, std::size_t... Ks, class... CanonicalSliceSpecifiers>
constexpr auto compute_all_sub_strides(
    LayoutMapping const& mapping,
    std::index_sequence<Ks...>,
    CanonicalSliceSpecifiers const&... slices
) noexcept -> array<IndexType, sizeof...(CanonicalSliceSpecifiers)> {
...
class submdspan_sub_strides_fn {
 ...
 public:
  ...
  /// @param slices the slice specifiers defining the submdspan
  /// @note Constraints <br>
  ///   @c sizeof...(CanonicalSliceSpecifiers) == LayoutMapping::extents_type::rank()
  /// @return an array of strides for the submdspan
  template <
      class LayoutMapping,
      class... CanonicalSliceSpecifiers,
      constraints<std::enable_if_t<sizeof...(CanonicalSliceSpecifiers) == LayoutMapping::extents_type::rank()>> =
          nullptr>
  constexpr auto operator()(LayoutMapping const& mapping, CanonicalSliceSpecifiers const&... slices) const noexcept
      -> array<IndexType, inverse_map_rank<CanonicalSliceSpecifiers...>.size()> {
    ...
    auto const sub_strides = submdspan_sub_strides_detail::compute_all_sub_strides<source_index_type>(
        mapping,
        std::make_index_sequence<sizeof...(CanonicalSliceSpecifiers)>{},
        slices...
    );
    // parasoft-end-suppress AUTOSAR-M8_5_2-a

    auto result = array<IndexType, inverse_map_rank<CanonicalSliceSpecifiers...>.size()>{};

    for (std::size_t i{}; i != inverse_map_rank<CanonicalSliceSpecifiers...>.size(); ++i) {
      result[i] = sub_strides[inverse_map_rank<CanonicalSliceSpecifiers...>[i]];
    }
