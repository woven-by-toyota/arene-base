...
#ifndef INCLUDE_GUARD_ARENE_BASE_ARENE_BASE_MUTEX_MUTEX_HPP_
#define INCLUDE_GUARD_ARENE_BASE_ARENE_BASE_MUTEX_MUTEX_HPP_
...
///
/// @brief A mutex suitable for use with clang's Thread Safety Analysis. Equivalent to @c std::mutex.
///
/// If Thread Safety Analysis is not supported by the current compiler, or the current stdlib already has a TSA
/// annotated @c std::mutex, then this is a simple alias to @c std::mutex. Otherwise it is a thin wrapper
/// around @c std::mutex which annotates it as a _capability_, but is otherwise identical to @c std::mutex.
///
...
using mutex = mutex_detail::selected_mutex;

}  // namespace base
}  // namespace arene

#endif  // INCLUDE_GUARD_ARENE_BASE_ARENE_BASE_MUTEX_MUTEX_HPP_
