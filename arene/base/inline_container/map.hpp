...
class inline_map
  ...
  class iterator_impl
    ...
    /// a const map
    using map_type = std::conditional_t<IsConst, inline_map const, inline_map>;
    ...
    using implicit_conversion_type = std::conditional_t<IsConst, non_const_iterator, non_constructible_dummy>;
 ...
 public:
  ...
  /// @brief map capacity
  static constexpr auto capacity = std::integral_constant<std::size_t, Capacity>{};
 ...
 private:
  ...
  using copy_assign_source = std::conditional_t<
  ...
  /// non_constructible_dummy
  using dummy_copy_assign_source = std::conditional_t<
      !std::is_copy_assignable<Compare>::value || !std::is_copy_constructible<value_type>::value,
      inline_map,
      non_constructible_dummy>;
  ...
  using move_assign_source = std::conditional_t<
  ...
  /// non_constructible_dummy
  using dummy_move_assign_source = std::conditional_t<
      !std::is_move_assignable<Compare>::value || !std::is_move_constructible<value_type>::value,
      inline_map,
      non_constructible_dummy>;
  ...
  using move_construct_source = std::conditional_t<
 ...
 private:
  ...
  friend auto value_at_index(inline_map_passkey, Self& self, std::uint16_t index)
      -> std::conditional_t<std::is_const<std::remove_reference_t<Self>>::value, value_type const&, value_type&> {
  ...
  ARENE_NODISCARD static auto do_at(Self&& self, OtherKey const& key)
      -> std::conditional_t<std::is_const<remove_reference_t<Self>>::value, mapped_type const&, mapped_type&> {
