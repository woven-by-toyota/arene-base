...
class inline_deque_internal_storage
    ...
    , private storage_wrapping_interface<inline_deque_internal_storage<T, Capacity, AllowWrapping>, AllowWrapping> {
 protected:
 ...
 private:
  ...
  using copy_constructor_arg =
      std::conditional_t<std::is_copy_constructible<T>::value, inline_deque_internal_storage, non_constructible_dummy>;
  ...
  using move_constructor_arg =
      std::conditional_t<std::is_move_constructible<T>::value, inline_deque_internal_storage, non_constructible_dummy>;
 ...
 protected:
  ...
  /// @pre <c>distance(in_begin, in_end) >= 0</c> and <c><= capacity()</c>, otherwise ARENE_PRECONDITION violation
  ...
  constexpr inline_deque_internal_storage(Iterator in_begin, Iterator in_end) noexcept(
    ...
    auto distance = ::arene::base::distance(in_begin, in_end);
  ...
  /// @pre If wrapping is not allowed, <c>size() < capacity()</c>, otherwise ARENE_PRECONDITION violation
  ...
  /// @pre <c>size() > 0</c>, otherwise ARENE_PRECONDITION violation
...
class inline_deque_internal_storage<T, Capacity, AllowWrapping, true>
 ...
 protected:
  ...
  constexpr inline_deque_internal_storage(Iterator in_begin, Iterator in_end) noexcept(noexcept(
    ...
    auto new_end = ::arene::base::copy(in_begin, in_end, this->begin());
...
class inline_deque_impl
  ...
  class cyclical_iterator : public inline_container_detail::iterator_interface<cyclical_iterator<IsConst>> {
   ...
   public:
    ...
    constexpr cyclical_iterator(
        std::conditional_t<IsConst, inline_deque_impl const&, inline_deque_impl&> deque,
        std::size_t idx
    ) noexcept
        : iterator_interface(),
          deque_(&deque),
   ...
   private:
    ...
    std::conditional_t<IsConst, inline_deque_impl const*, inline_deque_impl*> deque_;
 ...
 public:
  ...
  /// @pre <c>size() < Capacity</c>, otherwise ARENE_PRECONDITION violation
  ...
  /// @pre <c>empty() == false</c>, otherwise ARENE_PRECONDITION violation
...
class inline_deque
 ...
 public:
  ...
  /// @pre <c>init.size() <= capacity()</c>, otherwise ARENE_PRECONDITION violation
