...
/// @brief adapter for parameterized tests using @c arene::base::type_list
struct as_arene_type_list {
  template <typename... Ts>
  using type_list_t = ::arene::base::type_list<Ts...>;
};

/// @brief adapter for parameterized tests using @c ::testing::Types
struct as_gtest_type_list {
  template <typename... Ts>
  using type_list_t = ::testing::Types<Ts...>;
};

/// @brief adapter for parameterized tests using @c std::tuple
struct as_std_tuple {
  template <typename... Ts>
  using type_list_t = std::tuple<Ts...>;
};

/// @brief test suite parameter list covering all three type list kinds
