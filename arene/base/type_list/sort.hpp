...
struct sort_impl<ComparisonPredicate, First, Second> {
  ...
  /// @brief The sorted list
  using type =
      type_list<conditional_t<second_before_first, Second, First>, conditional_t<second_before_first, First, Second>>;
};
...
struct sort_impl<ComparisonPredicate, First, Second, Third, RemainingTypes...> {
  ...
  using first_element = conditional_t<                    //
      second_before_first,                                //
      conditional_t<third_before_second, Third, Second>,  //
      conditional_t<third_before_first, Third, First>>;

  /// @brief The second element in the first 3
  using second_element = conditional_t<
      second_before_first,
      conditional_t<third_before_second, Second, conditional_t<third_before_first, Third, First>>,
      conditional_t<third_before_first, First, conditional_t<third_before_second, Third, Second>>>;

  /// @brief The third element in the first 3
  using third_element = conditional_t<                    //
      third_before_first,                                 //
      conditional_t<second_before_first, First, Second>,  //
      conditional_t<third_before_second, Second, Third>>;
