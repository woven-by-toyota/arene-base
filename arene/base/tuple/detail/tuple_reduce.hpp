...
class tuple_reduce_fn  //
{
  ...
  static constexpr auto impl(std::index_sequence<Is...>, Tuple&& tuple, T&& init, Reduce& reduce) noexcept(false) -> T {
 ...
 public:
  ...
  constexpr auto operator()(Tuple&& tuple, T init, Reduce reduce) const noexcept(false) -> T {
    return impl(
