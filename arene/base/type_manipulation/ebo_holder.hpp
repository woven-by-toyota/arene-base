...
class ebo_holder {
 ...
 protected:
  ...
  /// @tparam T value to construct from
  /// @param arg argument to construct from
  ///
  /// Constructs value from @c arg.
  ///
  template <
      class T,
      constraints<
          std::enable_if_t<std::is_constructible<Value, T&&>::value>,
          std::enable_if_t<!std::is_same<ebo_holder, remove_cvref_t<T>>::value>> = nullptr>
  // NOLINTNEXTLINE(bugprone-forwarding-reference-overload)
  constexpr explicit ebo_holder(T&& arg) noexcept(std::is_nothrow_constructible<Value, T&&>::value)
