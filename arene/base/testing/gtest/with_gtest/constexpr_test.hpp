...
/// @brief GoogleTest test type used to conditionally invoke tests at compile-time
/// @tparam TestTag tag associated with a specific test implementation function
/// @tparam RunInConstexpr when @c true the test body is also evaluated at compile-time
///
template <class TestTag, bool RunInConstexpr>
class conditionally_constexpr_test : public ::testing::Test {
 public:
  auto TestBody() -> void override {
    /* The test_impl() function must be called at runtime before the compile-time dispatch,
     * otherwise gcc8 will incorrectly think test_impl() is not invocable in a constant expression. */
    test_impl<void>(TestTag{});
    static_cast<void>(std::integral_constant<bool, (test_impl<void>(TestTag{}), true)>{});
  }
};

/// @brief Specialization for the @c false case: the test body runs at runtime only.
///
template <class TestTag>
class conditionally_constexpr_test<TestTag, false> : public ::testing::Test {
 public:
  auto TestBody() -> void override { test_impl<void>(TestTag{}); }
};
...
/// @brief defines a test case that is conditionally invoked at compile-time
/// @param testsuite The GoogleTest testsuite name
/// @param testcase The GoogleTest testcase name
/// @param Condition A condition convertible to @c bool
///
/// Defines a @c TEST that conditionally invokes a @c constexpr test implementation
/// function. The implementation function is invoked at least once at runtime, and
/// possibly a second time at compile-time, depending on the @c Condition parameter.
///
/// ~~~{.cpp}
