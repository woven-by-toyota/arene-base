...
template <typename T>
class UpperBound : public testing::Test {};

TYPED_TEST_SUITE(UpperBound, iterator_category_types, );

/// @test In a sorted range, @c std::upper_bound finds the first element
/// ordered after 1.
///
CONSTEXPR_TYPED_TEST(UpperBound, ElementInRange) {
  auto const values = {0, 1, 2, 3, 4};
  auto const range = testing::demoted_view(TypeParam{}, values);

  auto const it = std::upper_bound(range.begin(), range.end(), 1);

  ASSERT_EQ(it, arene::base::next(range.begin(), 2));
}

/// @test In a sorted range, @c std::upper_bound finds the first element
/// ordered after 1 when it is the first element.
///
CONSTEXPR_TYPED_TEST(UpperBound, ElementIsFirstInRange) {
  auto const values = {2, 3, 4};
  auto const range = testing::demoted_view(TypeParam{}, values);
...
/// @test In a sorted range, @c std::upper_bound finds the first element
/// ordered after 1 when it is the last element.
///
CONSTEXPR_TYPED_TEST(UpperBound, ElementIsLastInRange) {
  auto const values = {-1, 0, 1, 2};
  auto const range = testing::demoted_view(TypeParam{}, values);

  auto const it = std::upper_bound(range.begin(), range.end(), 1);

  ASSERT_EQ(it, arene::base::next(range.begin(), 3));
}

/// @test In a sorted range with duplicate values, @c std::upper_bound finds the
/// first element ordered after 1.
///
CONSTEXPR_TYPED_TEST(UpperBound, DuplicateElementsInRange) {
  auto const values = {0, 1, 2, 2};
  auto const range = testing::demoted_view(TypeParam{}, values);
...
/// @test In a sorted range without the target element, @c std::upper_bound
/// finds the first element ordered after 1.
///
CONSTEXPR_TYPED_TEST(UpperBound, ElementNotInRange) {
...
/// @test In a partitioned range without the target element, @c std::upper_bound
/// finds the first element in the partition of element ordered after 1.
///
CONSTEXPR_TYPED_TEST(UpperBound, ElementNotInPartitionedRange) {
...
/// @test In a partitioned range, @c std::upper_bound
/// returns the last element if *no* elements are *not* ordered after 10.
///
CONSTEXPR_TYPED_TEST(UpperBound, NoElementsOrderedBefore) {
  auto const values = {0, 0, 4, 3, 2};
  auto const range = testing::demoted_view(TypeParam{}, values);

  auto const it = std::upper_bound(range.begin(), range.end(), 10);

  ASSERT_EQ(it, range.end());
}

/// @test In an empty range, @c std::upper_bound returns the last element.
///
CONSTEXPR_TYPED_TEST(UpperBound, EmptyRange) {
  auto const values = std::initializer_list<int>{};
  auto const range = testing::demoted_view(TypeParam{}, values);

  auto const it = std::upper_bound(range.begin(), range.end(), 0);
...
class less_than_comparable {
 ...
 public:
  ...
  friend constexpr auto operator<(less_than_comparable lhs, int rhs) -> bool { return lhs.value_ < rhs; }
};

/// @test @c std::upper_bound uses `operator<` for comparison
/// of the range element type with the input value type.
///
CONSTEXPR_TYPED_TEST(UpperBound, ValueIsDifferentType) {
  auto const values = {0, 1, 2, 3, 4};
  auto const range = testing::demoted_view(TypeParam{}, values);

  auto const it = std::upper_bound(range.begin(), range.end(), less_than_comparable{1});

  ASSERT_EQ(it, arene::base::next(range.begin(), 2));
}

/// @test @c std::upper_bound uses a custom comparison function object if one is provided.
///
CONSTEXPR_TYPED_TEST(UpperBound, WithCustomComparitor) {
  auto const values = {0, 1, 2, 3, 4};
  auto const range = testing::demoted_view(TypeParam{}, values);
  ...
  auto const it = std::upper_bound(range.begin(), range.end(), 1, less_equal{});
...
/// @test @c std::upper_bound uses a custom comparison function if one is provided.
///
CONSTEXPR_TYPED_TEST(UpperBound, WithCustomComparitorFunction) {
  auto const values = {0, 1, 2, 3, 4};
  auto const range = testing::demoted_view(TypeParam{}, values);

  auto const it = std::upper_bound(range.begin(), range.end(), 1, int_less_equal);

  ASSERT_EQ(it, arene::base::next(range.begin(), 1));
}

/// @test @c std::upper_bound performs at most log2(N) + O(1) comparisons.
///
TYPED_TEST(UpperBound, MeetsComplexityRequirement) {
...
/// @test @c std::upper_bound is @c noexcept if iterator operations and
/// application of the default comparison object are @c noexcept.
///
TEST(UpperBound, NoexceptIfIteratorOpsAndCompareAreNoexcept) {
  using iterator_type = int const*;

  ASSERT_TRUE(noexcept(std::upper_bound(iterator_type{}, iterator_type{}, 0)));
}
...
/// @test @c std::upper_bound is not @c noexcept if the comparison can throw.
///
TEST(UpperBound, NotNoexceptIfComparisonCanThrow) {
  using iterator_type = int const*;
  // one of the arguments must be 'declval', otherwise using GCC8 and C++17 will
  // *always* deduce 'noexcept' as 'true'
  ASSERT_FALSE(noexcept(std::upper_bound(std::declval<iterator_type>(), iterator_type{}, 0, throwing_cmp{})));
}
...
/// @test @c std::upper_bound is not @c noexcept if iterator operations can throw
///
TEST(UpperBound, NotNoexceptIfIteratorOperationsCanThrow) {
  auto const is_upper_bound_noexcept = [](auto iter) { return noexcept(std::upper_bound(iter, iter, 0)); };

  ASSERT_TRUE(is_upper_bound_noexcept(noexcept_test_iterator<throws_on::nothing>{}));

  ASSERT_FALSE(is_upper_bound_noexcept(noexcept_test_iterator<throws_on::construct>{}));
  ASSERT_FALSE(is_upper_bound_noexcept(noexcept_test_iterator<throws_on::assign>{}));
  ASSERT_FALSE(is_upper_bound_noexcept(noexcept_test_iterator<throws_on::deref>{}));
  ASSERT_FALSE(is_upper_bound_noexcept(noexcept_test_iterator<throws_on::minus>{}));
  ASSERT_FALSE(is_upper_bound_noexcept(noexcept_test_iterator<throws_on::plus_equals>{}));
}

/// @test all overloads of @c std::upper_bound are not invocable with input
/// iterator types or output iterator types
///
TEST(UpperBound, RequiresForwardIterator) {
  auto const is_invocable_with = [](auto category, auto... cmp) {
    using iterator_type = testing::demoted_iterator<int*, decltype(category)>;
    auto const upper_bound = FUNCTION_LIFT(std::upper_bound);
    return testing::simple_is_invocable_v<decltype(upper_bound), iterator_type, iterator_type, int, decltype(cmp)...>;
  };
...
/// @test @c std::upper_bound requires a binary predicate where the iterator
/// reference type is convertible for arg1 and the value argument is convertible
/// to arg2
///
TEST(UpperBound, RequiresBinaryPredicate) {
  auto const is_invocable_with = [](auto iter, auto value, auto... cmp) {
    auto const upper_bound = FUNCTION_LIFT(std::upper_bound);
    return testing::
        simple_is_invocable_v<decltype(upper_bound), decltype(iter), decltype(iter), decltype(value), decltype(cmp)...>;
  };
