...
template <typename T>
class MinElement : public testing::Test {};

TYPED_TEST_SUITE(MinElement, iterator_category_types, );

/// @test @c std::min_element returns an iterator to the smallest element, where
/// the smallest element is at position 0 of the input range.
///
CONSTEXPR_TYPED_TEST(MinElement, SingleSmallestAt0) {
  auto const values = {0, 1, 1};
  auto const range = testing::demoted_view(TypeParam{}, values);

  auto const it = std::min_element(range.begin(), range.end());

  ASSERT_EQ(it, arene::base::next(range.begin(), 0));
}

/// @test @c std::min_element returns an iterator to the smallest element, where
/// the smallest element is at position 1 of the input range.
///
CONSTEXPR_TYPED_TEST(MinElement, SingleSmallestAt1) {
...
/// @test @c std::min_element returns an iterator to the smallest element, where
/// the smallest element is at position 2 of the input range.
///
CONSTEXPR_TYPED_TEST(MinElement, SingleSmallestAt2) {
...
/// @test @c std::min_element returns an iterator to the first occurence of the
/// smallest element, if there are multiple, and where the first occurrence is at
/// position 0 of the input range.
///
CONSTEXPR_TYPED_TEST(MinElement, MultipleSmallestAt0) {
...
///
CONSTEXPR_TYPED_TEST(MinElement, MultipleSmallestAt1) {
...
/// @test @c std::min_element returns an iterator to the first occurence of the
/// smallest element, if there are multiple, and where the second occurence is
/// not adjacent to the first occurence.
///
CONSTEXPR_TYPED_TEST(MinElement, MultipleSmallestNotContiguous) {
...
/// @test @c std::min_element returns @c last if the range is empty.
///
CONSTEXPR_TYPED_TEST(MinElement, EmptyRange) {
...
/// @test @c std::min_element uses a custom comparator if one is provided.
///
CONSTEXPR_TYPED_TEST(MinElement, CustomComparator) {
  ...
  auto const it = std::min_element(range.begin(), range.end(), less_than_negated{});

  ASSERT_EQ(it, arene::base::next(range.begin(), 1));
}

/// @test @c std::min_element performs exactly max(N - 1 , 0) comparisons.
///
CONDITIONALLY_CONSTEXPR_TYPED_TEST(MinElement, MeetsComplexityRequirement, using_cpp17) {
...
/// @test @c std::min_element is @c noexcept if iterator operations and
/// application of the default comparison object are @c noexcept.
///
TYPED_TEST(MinElement, NoexceptIfIteratorOpsAndCompareAreNoexcept) {
  using iterator_type = int const*;

  ASSERT_TRUE(noexcept(std::min_element(iterator_type{}, iterator_type{})));
}

/// @test @c std::min_element is not @c noexcept if the comparison can throw.
///
TYPED_TEST(MinElement, NotNoexceptIfComparisonCanThrow) {
  using iterator_type = int const*;
  ...
  ASSERT_FALSE(noexcept(std::min_element(std::declval<iterator_type>(), iterator_type{}, cmp)));
}
...
/// @test @c std::min_element is not @c noexcept if iterator operations can throw
///
TYPED_TEST(MinElement, NotNoexceptIfIteratorOperationsCanThrow) {
  auto const is_algorithm_noexcept = [](auto iter) { return noexcept(std::min_element(iter, iter)); };
...
/// @test all overloads of @c std::min_element are not invocable with input
/// iterator types or output iterator types
///
TYPED_TEST(MinElement, RequiresForwardIterator) {
  auto const is_invocable_with = [](auto category, auto... cmp) {
    using iterator_type = testing::demoted_iterator<int*, decltype(category)>;
    auto const lifted = FUNCTION_LIFT(std::min_element);
...
/// @test @c std::min_element requires a binary predicate where the iterator
/// reference type is convertible for arg1 and for arg2
///
TEST(MinElement, RequiresBinaryPredicate) {
