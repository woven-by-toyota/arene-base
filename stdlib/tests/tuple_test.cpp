...
/// @test a tuple with a reference element type can be constructed from
/// a reference or a reference wrapper
CONSTEXPR_TEST(Tuple, UsableWithReferencesAndReferenceWrapperLike) {
  using Tup = std::tuple<int const&, int>;

  int value = 0;
  Tup tup{value, value};
  value = 1;
  ASSERT_EQ(std::get<0>(tup), 1);
  ASSERT_EQ(std::get<1>(tup), 0);
  ASSERT_EQ(std::get<int const&>(tup), 1);
  ASSERT_EQ(std::get<int>(tup), 0);

  Tup tup2{std::get<1>(tup), std::get<1>(tup)};
  value = 2;
  std::get<1>(tup) = 3;
  ASSERT_EQ(std::get<0>(tup2), 3);
  ASSERT_EQ(std::get<1>(tup2), 0);
  ...
  struct constexpr_reference_wrapper {
    ...
    constexpr operator int const&() const noexcept { return value; }
  };

  Tup tup3{constexpr_reference_wrapper{value}, value};
  value = 4;
  ASSERT_EQ(std::get<0>(tup3), 4);
  ASSERT_EQ(std::get<1>(tup3), 2);
