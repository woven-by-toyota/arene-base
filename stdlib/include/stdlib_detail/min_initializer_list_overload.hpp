...
#ifndef INCLUDE_GUARD_ARENE_BASE_STDLIB_INCLUDE_STDLIB_DETAIL_MIN_INITIALIZER_LIST_OVERLOAD_HPP_
#define INCLUDE_GUARD_ARENE_BASE_STDLIB_INCLUDE_STDLIB_DETAIL_MIN_INITIALIZER_LIST_OVERLOAD_HPP_
...
/// @brief Returns the smallest value in the @c std::initializer_list
...
/// @return Returns the smallest value in @c list using the comparison function @c comp to compare the values
///
...
template <
    ...
    arene::base::constraints<
        ...
        std::enable_if_t<arene::base::is_compare_v<Compare&, T&>>> = nullptr>
constexpr auto min(
  ...
  return *std::internal::relative_find(list.begin(), list.end(), comp);
}

/// @brief Returns the smallest value in the @c std::initializer_list
/// @tparam T The type of the values in the @c std::initializer_list
/// @param list The list of values to compare
/// @return Returns the smallest value in @c list
///
/// @pre <tt> list.size() > 0 </tt>
/// @pre <tt> std::is_copy_constructible<T>::value == true </tt>
/// @pre The expression <tt>comp(a, b)</tt> must be convertible to @c bool
...
constexpr auto min(initializer_list<T> list
) noexcept(noexcept(std::min(list, arene::base::algorithm_detail::operator_less))) -> T {
  return min(list, arene::base::algorithm_detail::operator_less);
}
...
#endif  // INCLUDE_GUARD_ARENE_BASE_STDLIB_INCLUDE_STDLIB_DETAIL_MIN_INITIALIZER_LIST_OVERLOAD_HPP_
