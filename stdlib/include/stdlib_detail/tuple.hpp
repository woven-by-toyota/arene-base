...
/// @brief handles reference binding when initializing a reference from a
///   forwarded argument
/// @tparam T the target type
///
/// This primary template forwards the argument unchanged.
template <class T, class = void>
class convert_reference_wrapper_like : public arene::base::identity {};

/// @brief handles reference binding when initializing a reference from a
///   forwarded argument
/// @tparam T the target type without the reference qualifier
///
/// When initializing a reference member of type @c T& or @c T&& from an xvalue
/// (e.g. the result of @c std::forward), [dcl.init.ref] requires
/// the initializer to be a direct binding and not involve a user-defined
/// conversion. If the forwarded type is a reference-wrapper-like proxy that
/// provides a conversion operator to @c T& or @c T&&, the xvalue causes the
/// direct binding path to be skipped, and the compiler may infer that the
/// lifetime of the reference is bounded by the lifetime of the temporary,
/// causing a compilation error.
///
/// This specialization ensures that the conversion to @c T& or @c T&& happens
/// explicitly via @c static_cast before the result is used to initialize
/// the reference member, producing a reference that satisfies [dcl.init.ref].
template <class T>
class convert_reference_wrapper_like<T, enable_if_t<is_reference_v<T>>> {
 public:
  // parasoft-begin-suppress AUTOSAR-A8_4_6-a-2  "This function object handles a proxy reference conversion"
  // parasoft-begin-suppress AUTOSAR-A8_4_5-a-2  "This function object handles a proxy reference conversion"
  // parasoft-begin-suppress AUTOSAR-A12_8_4-a-2 "This function object handles a proxy reference conversion"
  /// @brief convert the given value to @c T&
  /// @tparam U the type of the value
  /// @param value the value to convert
  /// @return an lvalue reference
  template <class U>
  constexpr auto operator()(U&& value) noexcept -> T {
...
class tuple_leaf : arene::base::ebo_holder<integral_constant<size_t, I>, remove_const_t<T>> {
  /// @brief tag type used to access the value within 'ebo_holder'
  using base_tag_type = integral_constant<size_t, I>;

 public:
  using arene::base::ebo_holder<base_tag_type, remove_const_t<T>>::ebo_holder;
...
/// @brief alias to the tuple leaf type at index I of a tuple
/// @tparam I leaf index
/// @tparam Tuple tuple type, specialization of std::tuple
template <std::size_t I, class Tuple>
using tuple_leaf_t = tuple_leaf<I, tuple_element_t<I, Tuple>>;
...
class tuple_impl<index_sequence<I...>, arene::base::type_list<T...>>
 ...
 public:
  ...
  constexpr explicit tuple_impl(UTypes&&... args)  //
      ...
      : tuple_leaf<I, T>{convert_reference_wrapper_like<T>{}(std::forward<UTypes>(args))}... {}
...
class tuple : tuple_detail::tuple_impl<index_sequence_for<Types...>, arene::base::type_list<Types...>> {
  // parasoft-begin-suppress AUTOSAR-A2_10_1-e "False positive: 'impl_type' does not hide an identifier in 'tuple'"
  ...
  /// @note The SFINAE constraint is to prevent ambiguity for a tuple of tuples.
  ///   Without the constraint, ADL finds this friend through the associated
  ///   classes of the template arguments, causing the inner tuple instantiations'
  ///   friends to also be viable candidates via implicit derived-to-base
  ///   conversion.
  template <
      size_t I,
      class Tuple,
      arene::base::constraints<std::enable_if_t<std::is_same<Tuple, tuple>::value>> = nullptr>
  friend constexpr auto __get_tuple_impl(Tuple& tup) noexcept -> tuple_detail::tuple_leaf_t<I, Tuple>& {
    return static_cast<tuple_detail::tuple_leaf_t<I, Tuple>&>(static_cast<impl_type&>(tup));
  }
  // parasoft-end-suppress AUTOSAR-M3_3_2-a
  // parasoft-end-suppress AUTOSAR-A11_3_1-a
  ...
  // parasoft-begin-suppress AUTOSAR-A2_10_1-e "False positive: 'are_all_arguments_v' does not hide private member
  // 'are_all_arguments_v' in 'tuple_impl'"
  /// @brief determine if a trait predicate is @c true for all pairs of @c T...
  ///   and @c U...
  /// @tparam Trait binary predicate type trait to transform @c T... with @c U...
  /// @tparam U types paired with @c T...
  ...
  // parasoft-begin-suppress AUTOSAR-A2_10_1-e "False positive: 'type_index_sequence' does not hide anything"
...
// parasoft-begin-suppress AUTOSAR-A2_7_3-a
